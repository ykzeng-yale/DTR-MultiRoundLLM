...
def build(out_dir, spec_path=None, source_path=None, law_dir=None):
    ...
    config.update(dataset_sha256=digest(tasks), source_code_sha256=digest(source_hashes()),
                  ...
                  max_completion_tokens=PLANNING_INVENTORY["reserved_completion_tokens"], max_seconds=1560)
    config["e14"] = {
    ...
    (out_dir / "CHANGE_NOTES.md").write_text(CHANGE_NOTES.format(
        ...
        source_rel=Path(source_path).relative_to(ROOT), source_sha=source_sha,
        ...
        roster=", ".join(str(t) for t in ROSTER), held=", ".join(str(t) for t in HELD_IDS)), encoding="utf-8")
    ...
    manifest = {
        ...
        "source_pins": {"path": str(Path(source_path).relative_to(ROOT)), "file_sha256": source_sha,
        ...
        "builder": {"script": "scripts/build_e14_release.py", "script_sha256": file_sha(__file__),
                    "deterministic": True,
...
def main(argv=None):
    ...
    a = p.parse_args(argv)
    kw = dict(spec_path=a.spec, source_path=a.source, law_dir=a.law_release)
    if a.verify:
