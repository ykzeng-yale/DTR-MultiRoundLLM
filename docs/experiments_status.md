...
3. (Done by the lead: LEAD-E0-RUNNER-02 `5d80ce2` repaired the killpg race; the worker verified the fix.) If the lead releases the clean-checkout test repairs in §5.2: make every source-cache-dependent builder test skip with a reason, as the frame-review tests already do. Pin the MBPP cache acquisition (URL plus sha256) in one documented command. Replace the absolute E2 `permitted_path` with a repository-relative path. Show each test failing on a clean worktree before the fix and passing or skipping after it.
