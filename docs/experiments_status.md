...
**Last updated: 2026-09-25T07:14:15Z** — **Heartbeat (watcher tick 2026-09-25T07:14:15Z).** Last lead commit processed: `a2b3cd6f63cebc719d59f8b2de64e487c70cb517`. **LEAD-POLICY-09 ACCEPTED** (processed `a2b3cd6f63cebc719d59f8b2de64e487c70cb517`, read at 07:13:34Z). HOLD on real collection accepted. Provenance, checked read-only: the output `plan_sha256` and `source_sha256` equal `git show 01ef056:<path>` blob hashes; the source is unchanged at HEAD; raw and final `rows` are identical; only `freeze_commit` (raw `01ef056b2cf5…`, not a real commit) plus `metadata_correction` differ; and the result and raw files hash to the values in the validation record. **Third independent method:** a direct trinomial sum in log space (no convolution, no conditional binomial) reproduced all 18 benefit and futility cells to max |diff| 2.3e-13, with total mass 1 (0.22 s CPU, results/ untouched, nothing written to the repo). The quoted ranges hold (δ=0.20: 75.99–90.77% at n=544 and 1.96–12.90% at n=198; δ=0.10 at n=198 ≤0.157%), with one last-digit rounding nit: the n=544, δ=0.10 maximum is 0.922488%, so 0.922% rather than 0.923%. Also from the grid: futility is ≤0.0071% in every cell, and every δ=0.05 cell is ≥99.987% inconclusive. Run/lease: none; no rerun or new job. Watcher: **running**, re-armed each tick, wakes on every lead commit and at most every 30 min while this desktop session is open; the separate half-hourly scheduler has not fired since 2026-09-23T08:10:19Z. Run/lease: **none**. E14 N1/S1 batch: **NO-GO accepted** (LEAD-POLICY-06); source/mock kept as an instrument artifact with no execution allowance. Next bounded action: none open on the worker side; **blocker** for any experiment work is a new lead-owned development design: action class, family frame, comparators, endpoint, inference and finite local cap.
