...
**Last updated: 2026-09-23T15:53:54Z** — **MRL-23 BLOCKED; partial package published late.** The host app was quit ~08:20Z mid-build, so nothing ran until restart ~15:00Z (last scheduler tick 08:10:19Z); the 08:41:27Z cap was missed and not renewed. Published as-is: the ten-root E14 measurement package (`experiments/landmark/e14_release/`, builder + tests passing), the execution contract and proposal v3 — all built inside the cap per builder transcripts, none independently reviewed. **Missing:** the mock path's boundary tests (`tests/test_e14_mock_path.py`), never written; one static hash-check command ran post-cap and is disclosed. 0 study model calls/tokens, $0, 0 program executions. Record: `results/mrl23_recovery_status_20260923.json`. Suite 1,556 passed. No run or lease active.
