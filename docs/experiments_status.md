...
**Last updated: 2026-09-26T04:17:30Z** — **Heartbeat (watcher tick 2026-09-26T04:17:30Z).** Last lead commit processed: `95a4fd73a7c103ea42d4a3afabf97344fd22f293`. **LEAD-PORT-02 acceptance received** (processed `95a4fd73a7c103ea42d4a3afabf97344fd22f293` at 04:17:08Z). The lead's second-host result (98 passed / 4 explicit skips / 0 failed; the actual Homebrew 3.14 E2 prefix is the only field that differs) matches the worker's alt-cache emulation. No further portability job was released, and none was taken. Stated boundaries accepted: skips and projections are not committed-package reproduction or original-run replay. The status rows are updated (`9559c42`). Watcher: **running**, re-armed each tick, wakes on every lead commit and at most every 30 min while this desktop session is open; the separate half-hourly scheduler has not fired since 2026-09-23T08:10:19Z. Run/lease: **none**. E14 N1/S1 batch: **NO-GO accepted** (LEAD-POLICY-06); source/mock kept as an instrument artifact with no execution allowance. Next bounded action: none open on the worker side; **blocker** for any experiment work is a new lead-owned development design: action class, family frame, comparators, endpoint, inference and finite local cap.
...
| Open worker objections | **None.** LEAD-POLICY-12 adopted the LEAD-POLICY-11 one-public-case objection (`3253fe2`). The worker's `os.killpg` race report (`a7261c5`) was repaired by the lead in LEAD-E0-RUNNER-02 (`5d80ce2`), and the worker verified the repair (§5.1). The worker's clean-checkout report (`e250b8f`) was released as LEAD-PORT-01 (`7c5ed08`) and **completed** in `82b9ff7`/this commit. A clean worktree now gives 0 failed, 0 errors and 39 explicit skips (§5.2). |
...
| Local check (not a job) | Full test suite at `198f1fc`: 1,663 passed, 1 failed (the killpg race). After the lead's repair `5d80ce2`, the suite gives **1,666 passed, 0 failed** on the worker host, in three consecutive runs. A clean worktree of `5d80ce2` gives 6 failed, 7 errors and 28 skipped (§5.2). | 2026-09-25T16:34:03Z. Host re-runs after `5d80ce2` was processed at 19:18:06Z; clean-worktree run completed 19:22:15Z | [COORDINATION.md](../COORDINATION.md) |
...
| LEAD-PORT-02 | 01:16 UTC header; commit 01:17:53Z (`c43fe39`) | LEAD-PORT-01 accepted for the explicit-skip/fail-closed scope. On a populated host with only the alternate cache and Homebrew Python 3.14, the lead saw 94 passed and 3 failed, all from path binding. Repair: separate committed-binding byte reproduction from a labeled alternate-host projection. One CPU, 10 min, no download, $0. | ACCEPTED/RUNNING at 01:19:31Z (`0856890`). COMPLETED: repair `1c807c3`. On an alt-cache-only worktree, before: 2 failed; after: 0 failed, 4 explicit skips. Populated checkout: 1,677/1,677. 920 frozen files unchanged. [Evidence](../results/lead_port02_20260926/summary.json). **Lead ACCEPTED** within its source/test scope (`95a4fd7`, 04:15 UTC). The lead's second host gave 98 passed, 4 explicit skips, 0 failed; its actual Homebrew 3.14 E2 prefix is the only field that differs, and rebinding it gives exact bytes. No further portability job was released. |
| LEAD-PORT-01 | 22:13 UTC header; commit 22:15:28Z (`7c5ed08`) | Proceed with one source-only clean-checkout repair: explicit missing-source skips with fail-closed builders; the inventory claim scoped to committed files; E2 portable without rewriting executed bytes. One CPU, 15 min, no download, $0. The manuscript was corrected to E14 NO-GO. | ACCEPTED/RUNNING at 22:16:25Z (`6449340`). COMPLETED in `82b9ff7` and the receipt commit; before/after evidence in [results/lead_port01_20260925](../results/lead_port01_20260925/summary.json) |
| Stage-residual hold | `ed61bc7` | Held until there are provenance-bound stage records or an authorized replay. | Same |
| LEAD-E0-RUNNER-02 | 19:16 UTC header; commit 19:17:46Z (`5d80ce2`) | A receipt-preserving cleanup repair to the lead-owned bounded regularization runner, fixing the worker-reported killpg EPERM race. An operational fix to a closed synthetic run's tool: no rerun, no release. The lead reports 26/26 affected tests and six unrelated full-suite failures on the lead host. | ACCEPTED. Verified: runner file 26/26; full suite 1,666/1,666 three times on the worker host. The lead host's six failures are host dependencies in worker-authored tests (§5.2) |
...
| 2 | **killpg EPERM race** in scripts/run_regularization_comparison.py, lines 169 and 176, a lead-owned runner. test_fast_raw_writer_cannot_be_reported_complete_above_cap failed once in the full suite (1,663 passed, 1 failed). It passes 6/6 when run alone, and its file passes 24/24. Consequence: the supervisor can raise before it writes its final receipt. Suggested fix: catch ProcessLookupError/PermissionError, then poll/wait and record the exit, and add a regression test. (An observation from this page's rebuild, not part of the `a7261c5` report: the STOP runner, scripts/run_stop_anchor_comparison.py:206, already wraps killpg in try/except OSError.) | **Resolved** by LEAD-E0-RUNNER-02 (`5d80ce2`): both cleanup sites now call a bounded `_stop_child_group` helper, and two regression tests force the EPERM branch. The worker verified it: runner file 26/26; full suite 1,666/1,666 three times on the worker host. | `a7261c5`, `6cb0931`, `5d80ce2` |
...
- **Clean-checkout reproducibility (found 2026-09-25 while verifying `5d80ce2`).** A clean `git worktree` of `5d80ce2`, using the same interpreter, gives **6 failed, 7 errors, 1,625 passed and 28 skipped**. The worker host's own checkout passes all 1,666. There are three root causes, all in worker-authored tests and builders:
  - **Repaired under LEAD-PORT-01 (`7c5ed08`, completed 2026-09-25).**
    - Clean worktree before (`7c5ed08`): 6 failed, 7 errors, 28 skipped. After (`82b9ff7`): **1,634 passed, 0 failed, 0 errors, 39 skipped**. The 11 new skips each state that the pinned MBPP source is absent; the 28 older skips are unchanged.
    - Populated checkout: **1,673 passed, 0 failed** (the 1,666 plus 7 new tests, which include fail-closed builder/verify tests).
    - All 920 tracked files under `experiments/` and `results/` are byte-identical; no builder, fixture or release file changed.
    - The seed-inventory claim now covers committed files: the 13 committed logs ⊆ the host-local 19-log record, and they equal the lead's 13-log Git-blob inventory; both records are kept as they were.
    - E2 uses a runtime-rebound projection, and the executed fixture still fails closed off its bound paths.
    - Remaining gap, kept visible: rebuilding the E14 v1/v2 and dev-release v3 packages still needs the external pinned file. See [mbpp_source_acquisition_20260925.md](mbpp_source_acquisition_20260925.md); no download was made.
    - Evidence: [summary](../results/lead_port01_20260925/summary.json).
  - **LEAD-PORT-02 (`c43fe39`, completed 2026-09-26, repair `1c807c3`).** Two contracts are now separate tests. (1) Committed-binding byte reproduction: the E14 v1 rebuild and `verify` run from `work/sources/mbpp_full.jsonl`, and E2 in the executing checkout with the recorded uv CPython 3.12.13 prefix. (2) A labeled alternate-host projection rebinds only the declared fields (the E14 v1 source-path line with its derived notes hash; the E2 private-spec path hash and interpreter prefix) and requires exact bytes; any other raw difference fails. `verify` from the alternate cache without projection fails closed. On an alt-cache-only worktree imitating the lead host, the suite goes from 2 failed to 0 failed with 4 explicit skips. Limitations stay visible: a skip or projection is not committed-byte reproduction, and the Homebrew-3.14 lead host is emulated here; the lead then observed it directly. [Evidence](../results/lead_port02_20260926/summary.json). **Accepted by the lead in `95a4fd7`.** A clean checkout still cannot reproduce the committed E14 release without the external pinned MBPP source, and the original E2 execution stays bound to its recorded environment.
  - (a) The pinned MBPP source cache `work/task_sources/mbpp_full_20260920_4700efb9/mbpp.jsonl` is gitignored and host-local. Without it, 3 E14 v1 builder tests fail (`SystemExit`/`StopIteration`), the E14 v2 committed-bytes rebuild fails, 7 dev-release-v3 builder tests error at setup (`FileNotFoundError`), while 27 frame-review tests (5 MRL-15, 22 ranks 21–40) and 1 v2 portability test skip cleanly. So the committed E14 v1/v2 and dev-release v3 packages **cannot be rebuilt from the repository alone**.
  - (b) The ignored `work/` call logs make the seed-inventory test fail, as already known.
  ...
  These account for the lead host's six failures (`5d80ce2` review). No repair is released; the proposed fix is in §8.
...
3. (Done.) The lead repaired the killpg race (LEAD-E0-RUNNER-02 `5d80ce2`), and the worker completed the clean-checkout test repairs (LEAD-PORT-01, `82b9ff7`). Remaining: rebuilding the three packages from a clean checkout needs the pinned external MBPP file, and a download needs its own lead review.
