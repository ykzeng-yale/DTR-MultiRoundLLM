# Adapted from ykzeng-yale/DTR-AgentEvals experiments/common/sandbox.py, itself adapted from
# ykzeng-yale/ICLR-WinRatioAgentEval experiments/local_stream/sandbox.py (same author).
# (same author; audited Seatbelt sandbox + sentinel verifier). Changes: sandbox
# base dir renamed to dtrmr_sbx; sentinel prefix renamed; per-run read isolation inside the
...
def sandbox_base_dir() -> str:
    ...
    d = os.path.join(os.path.realpath(tempfile.gettempdir()), 'dtrmr_sbx')
