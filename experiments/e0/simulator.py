...
def true_value(Kk: np.ndarray, pol: np.ndarray, T: int) -> float:
    """Exact E[Y] under `pol`. Verified against a 2,000,000-episode Monte Carlo to
    0.00 MC SE (test_e0.py::test_dp_matches_monte_carlo)."""
    V = Vtab(Kk, pol, T)
...
def true_blips_posterior(Kk: np.ndarray, T: int, continuation: np.ndarray = UNIF5) -> dict:
    ...
    state. Because ease and error type both shift the initial state, observing
...
def optimal_rule(Kk: np.ndarray, T: int, observable: str = 'state') -> dict:
    """Exact optimal regime by backward induction.

    `observable='state'` gives the rule available to a decision maker who sees S_t
    only -- the realistic case, since Z is latent. The (E, Z)-aware optimum is a
    strictly higher ceiling and is reported separately so the cost of not seeing the
    latents is explicit rather than hidden inside a regret number.
    """
    if observable == 'full':
        V = np.zeros((T + 2, NE, 2, NS))
        for e in range(NE):
            ...
            for e in range(NE):
                for z in (0, 1):
                    for s in range(NS):
                        ...
                        V[t, e, z, s] = qs[b]
        return rule
    # state-only: exhaustive over deterministic rules is 5^(NS*T); use the marginal
    # posterior-weighted Q at each (t, s) with a uniform continuation, which is the
    # object an estimator can actually target.
    rule = {}
...
def simulate(n_tasks: int, runs: int, T: int, Kk: np.ndarray, beh: np.ndarray, rng,
    ...
    replicate. The analyst observes only the class. This violates coarsening
    sufficiency -- the assumption that the outcome depends on the message only
    through its class -- so the estimand becomes a property of the template
    distribution the experimenter happened to write. sigma = 0 restores the
    assumption exactly.
    """
    n = n_tasks * runs
